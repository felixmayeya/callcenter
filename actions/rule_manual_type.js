...
module.exports = function($) {
	...
	* @author fanxd
	*/
	$.get("/rules/manual/type", function *(next) {
		var self = this;
		//将session放入ejs渲染数据中
		...
		yield function(callback) {
			ruleManualTypeModel.getRowsCount(params, function(err, count) {
				if(!err){
		...
		yield function(callback) {
			ruleManualTypeModel.getPagedRows(params, offset, limit, {
				created: -1
			}, function(err, rows) {
				if (err) {
					status = 400;
		...
		yield self.render('rules/manual_type', result);
	...
	*@author fanxd
	*/
	$.post("/rules/manual/type", function *(next) {
		var self = this;
		//将session放入ejs渲染数据中
		self.state.session = self.session;
		//admin 或 manager 角色才可访问
		...
		var ruleManualTypeModel = this.model("rule_manual_type");
		yield function(callback) {
			ruleManualTypeModel.createRow(params, function(err, row) {
				if (!err && row) {
					result = {
						app: row
					};
	...
	*@author fanxd
	*/
	$.put("/rules/manual/type/:id", function *(next) {
		var self = this;
		//将session放入ejs渲染数据中
		self.state.session = self.session;
		//admin 或 manager 角色才可访问
		...
		var ruleManualTypeModel = this.model("rule_manual_type");
		yield function(callback) {
			ruleManualTypeModel.findOneAndUpdateRow(params, _params, function(err, row) {
				if (!err && row) {
					result = {
						work: row
					};
	...
	*@author fanxd
	*/
	$.delete("/rules/manual/type/:id", function *(next) {
		var self = this;
		//将session放入ejs渲染数据中
		self.state.session = self.session;
		//admin 或 manager 角色才可访问
		...
		var ruleManualTypeModel = this.model("rule_manual_type");
		yield function (callback) {
			ruleManualTypeModel.deleteRow(params, function(err, row) {
				if (!err && row) {
					result = {
						app: row
					};
	...
	* @author fanxd
	*/
	$.get("/rules/manual/type/validate/:name", function *(next) {
		var self = this;
		//将session放入ejs渲染数据中
		self.state.session = self.session;
		//admin 或 manager 角色才可访问
